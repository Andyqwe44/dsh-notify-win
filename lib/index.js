...
export default {
  ...
  apply(ctx, config) {
    ...
    const fire = (kind, title, body, sessionId) => {
      const now = Date.now()
      const key = sessionId ?? '\u0000any'
      ...
      runNotify(kind, title, body).catch((error) => {
        console.error('[dsh-notify-win] notify failed:', error)
      })
    }

    const runNotify = async (kind, title, body) => {
    ...
    ctx.on('agent/status', (payload) => {
      ...
      const sessionId = payload?.agent?.session?.id ?? payload?.agent?.session?.header?.id
      fire('done', cfg.doneTitle, cfg.doneBody, sessionId)
    })
      ...
      if (req?.agent === undefined || isRootAgent(req.agent)) {
        const reason =
          typeof req?.reason === 'string' && req.reason.length > 0
            ? req.reason
            : cfg.questionBody
        fire('question', cfg.questionTitle, reason, req?.agent?.session?.id)
      }
      return next()
    })
    ...
    ctx.on('tools/execute', (exec, next) => {
      if (exec?.name === 'ask_user_question') {
        fire('question', cfg.askTitle, cfg.askBody, exec?.agent?.session?.id)
      }
      return next()
    })
  },
