...
export default {
  ...
  apply(ctx, config) {
    ...
    const fire = (kind, title, body) => {
      const now = Date.now()
      if (now - lastNotify < cfg.dedupMs) return
      lastNotify = now
    ...
    ctx.on('agent/status', (payload) => {
      ...
      fire('done', cfg.doneTitle, cfg.doneBody)
    })
      ...
      if (req?.agent === undefined || isRootAgent(req.agent)) {
        const reason =
          typeof req?.reason === 'string' && req.reason.length > 0
            ? req.reason
            : cfg.questionBody
        fire('question', cfg.questionTitle, reason)
      }
      return next()
    })
    ...
    ctx.on('tools/execute', (exec, next) => {
      if (exec?.name === 'ask_user_question') {
        fire('question', cfg.askTitle, cfg.askBody)
      }
      return next()
    })
  },
